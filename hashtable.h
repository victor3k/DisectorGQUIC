...
int timeout = 60;
  ...
  for(i = 0; i<SIZE; i++){
  	...
  	timeout = 60;
  	if(hashArray[i]->idle_connection_state != -1 && hashArray[i]->idle_connection_state != 30 && hashArray[i]->idle_connection_state != 0){
    	timeout = hashArray[i]->idle_connection_state;
  	}

    // Si ha pasado 1 minuto o mas desde la llegada del ultimo paquete, se borra la sesion
    if((hashArray[i]->time_last_pkt + timeout < time) && (hashArray[i]->time_last_pkt != -1)){
